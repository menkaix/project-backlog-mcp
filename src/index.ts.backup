...
import { TOOL_PERMISSIONS, MCPSSEMessage } from './types.js';
...
class BacklogMCPServer {
  ...
  private setupExpressApp() {
    ...
    apiRouter.get('/mcp/sse', authMiddleware, async (req, res): Promise<void> => {
      const requestId = (req as any).requestId;
      const authToken = (req as any).authToken;
      const clientInfo = (req as any).clientInfo;
    ...
    apiRouter.post('/mcp/sse/send', authMiddleware, async (req, res): Promise<void> => {
      const requestId = (req as any).requestId;
      const authToken = (req as any).authToken;
      ...
      try {
        ...
        if (message.method) {
          ...
          try {
            const response = await handleMCPMessage(message, authToken, requestId);
    ...
    apiRouter.post('/mcp/sse/broadcast', authMiddleware, async (req, res): Promise<void> => {
      const requestId = (req as any).requestId;
      const authToken = (req as any).authToken;
    ...
    apiRouter.get('/mcp/sse/stats', authMiddleware, (req, res): void => {
      const authToken = (req as any).authToken;
